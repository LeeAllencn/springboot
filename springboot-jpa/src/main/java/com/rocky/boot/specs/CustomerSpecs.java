...
public class CustomerSpecs {
	...
	public static <T> Specification<T> byAuto(final EntityManager entityManager, final T example) { //1 可以用于任意实体类
		...
		return new Specification<T>() {
			...
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				EntityType<T> entity = entityManager.getMetamodel().entity(type);//4 获得实体类的EntityType，以获取实体类的属性
				
				for (Attribute<T, ?> attr : entity.getDeclaredAttributes()) {//5 对实体类的所有属性循环
					Object attrValue = getValue(example, attr); //6 获取某个属性值
					if (attrValue != null) {
						if (attr.getJavaType() == String.class) { //7 属性值为字符类型
							if (!StringUtils.isEmpty(attrValue)) { //8 属性值不为空
			...
			/**
			 * 13 获取实体类的当前属相的SingularAttribute，SingularAttribute包含的是实体类的某个单独属性
			 */
